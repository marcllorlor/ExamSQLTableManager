...
namespace Gestio_ClCicles.CLASSES
{
    public class ClModulsSqlServer
    {
        ...
        {
            Boolean xb = bd.InserirDades("INSERT INTO tbModuls(idCicle, idModul, nomModul, nHores) VALUES('" + idCicle + "', '" + idModul + "','" + nomModul + "', " + nHores +")");
            return xb;
        }

        public Boolean modificarModul()
        {
            Boolean xb = bd.ModificarDades("UPDATE tbModuls SET idCicle='" + idCicle + "', nomModul ='" + nomModul + "',nHores = " + nHores +" WHERE (idModul='" + idModul + "')");
            return xb;
        }
        ...
        public Boolean getModul()
        {
            ...
            String xsql = "SELECT idModul, nomModul, idCicle, nHores FROM tbModuls WHERE idModul = '" + idModul + "';";
        ...
        public void llistaModuls(ref DataSet dset, int ordre)
        {
            ...
            switch (ordre)
            {
                case 0: xsql = "SELECT idModul, nomModul, idCicle, nHores FROM tbModuls ORDER BY idModul"; break;
                case 1: xsql = "SELECT tbModuls.idModul, tbModuls.nomModul, tbModuls.idCicle , tbCicles.nomCicle, tbModuls.nHores FROM tbModuls LEFT JOIN tbCicles ON tbCicles.idCicle = tbModuls.idCicle ORDER BY tbModuls.nomModul"; break;
            }
            bd.Consulta(xsql, ref dset);
        }
